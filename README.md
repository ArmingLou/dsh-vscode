...
- 📂 **File jumps**: clicking any file path in the panel — paths in model replies, produced-file chips, and tool-call rows such as `read · test/bridge/a.ts` / `Edit · README.md` — opens the file in the current VS Code window (instead of the OS default application). **Two-layer guarantee**: DOM interception of the entry elements plus a transport-level takeover of the `host.openPath` RPC that every DSH file-open action funnels through — the request never reaches the backend (no system-default app), is rerouted to the extension host, and DSH receives a forged success response, so any future file-open UI in DSH is covered automatically;
